console.log ('snacks');

// creare un array con i nomi in list per il party
const arraynames = [ "Marco", "Vaiva", "Mick", "Nastya", "Fabio","Gaya", "Riccardo", "Austeya", "Martyna" ];

// chiedere all'utente il proprio nome
const username = prompt ('inserisci il tuo nome') //string or nun

// const invitato = arraynames  

for (let i = 0; i < arraynames.length; i++) {

    // const nomecorrente = arraynames [i]

    if (username === arraynames [i]) {
        console.log ('ciao ' + username + ' puoi entrare al paty')
        
    } else if (username != arraynames [i] ){
        console.log ('ci dispiace ma il tuo nome non è sulla lista')
        
    }else {

    }
    

}

